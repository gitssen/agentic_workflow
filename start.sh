...
./venv/bin/uvicorn backend.main:app --host 0.0.0.0 --port 8000 --reload --log-level error >> logs/agent.log 2>&1 &
BACKEND_PID=$!
...
cd frontend && npm run dev -- -H 0.0.0.0 >> ../logs/agent.log 2>&1 &
FRONTEND_PID=$!

echo "--------------------------------------------------------"
echo "Backend:  http://192.168.1.100:8000"
echo "Frontend: http://192.168.1.100:3000"
echo "Combined Logs: logs/agent.log"
echo "--------------------------------------------------------"
